...
static int simplexSeed = 0x9abc8341;
...
float noise1d(float x, out float dnoise_dx)
{
  int i0 = floor(x);
  int i1 = i0 + 1;
  float x0 = x - i0;
...
float4 PsMesh(VsMeshOut p) : SV_Target
{
  ...
  float3 orgNormal = normalize(p.normal);
  ...
  return 0.2 * diffuse * float4(0.45, 0.4, 0.3, 1) + 0.3 * specular * float4(1,1,1,1);
}
